...
var BackendCalendar = {
    // :: CONSTANTS
    FILTER_TYPE_PROVIDER: 'yogiccoach',
    ...
    initialize: function(defaultEventHandlers) {
        ...
        if (GlobalVariables.availableProviders.length > 0) {
            ...
            $.each(GlobalVariables.availableProviders, function(index, provider) {
                ...
                optgroupHtml += '<option value="' + provider['fld_id'] + '" '
                        + 'type="' + BackendCalendar.FILTER_TYPE_PROVIDER + '" '
                        + 'google-sync="' + hasGoogleSync + '">'
                        + provider['fld_first_name'] + ' ' + provider['fld_last_name']
                        + '</option>';
            });
    ...
    bindEventHandlers: function() {
        ...
        $(document).on('click', '.edit-popover', function() {
            ...
            if (BackendCalendar.lastFocusedEventData.data.is_unavailable == false) {
                ...
                $dialog.find('#select-provider').val(appointment['id_users_provider']);
                $dialog.find('#select-customer').val(appointment['id_users_customer']);
                $dialog.find('#appointment-notes').val(appointment['notes']);
                // Set the start and end datetime of the appointment.
                var startDatetime = Date.parseExact(appointment['start_datetime'],
                        'yyyy-MM-dd HH:mm:ss');
        ...
        $(document).on('click', '.delete-popover', function() {
            ...
            if (BackendCalendar.lastFocusedEventData.data.is_unavailable == false) {
                ...
                messageButtons['OK'] = function() {
                    var postUrl = GlobalVariables.baseAPIUrl + 'ajax_delete_appointment';
                ...
                var postUrl = GlobalVariables.baseAPIUrl + 'ajax_delete_unavailable';
        ...
        $('#manage-appointment #save-appointment').click(function() {
            ...
            var appointment = {
                ...
                'id_users_provider': $dialog.find('#provider-id').val(),
                'id_users_customer': $dialog.find('#select-customer').val(),
        ...
        $('#enable-sync').click(function() {
            if ($('#enable-sync').hasClass('enabled') === false) {
                ...
                var authInterval = window.setInterval(function() {
                    ...
                    if (windowHandle.document !== undefined) {
                        if (windowHandle.document.URL.indexOf(redirectUrl) !== -1) {
                            ...
                            var postUrl = GlobalVariables.baseAPIUrl + 'ajax_get_google_calendars';
                            var postData = {
                                'csrfToken': GlobalVariables.csrfToken,
                ...
                $.each(GlobalVariables.availableProviders, function(index, provider) {
                    if (provider['fld_id'] == $('#select-filter-item').val()) {
                        provider['settings']['google_sync'] = '0';
                        provider['settings']['google_token'] = null;

                        BackendCalendar.disableProviderSync(provider['fld_id']);
        ...
        $('#select-service').change(function() {
            ...
            $.each(GlobalVariables.availableProviders, function(indexProvider, provider) {
                $.each(provider.services, function(indexService, serviceId) {
                    ...
                    if (serviceId == sid) {
                        var optionHtml = '<option value="' + provider['fld_id'] + '">'
        ...
        $('#select-calendar').click(function() {
            var postUrl = GlobalVariables.baseAPIUrl + 'ajax_select_google_calendar';
            var postData = {
                'csrfToken': GlobalVariables.csrfToken,
                'provider_id': $('#select-filter-item').val(),
    ...
    refreshCalendarAppointments: function($calendar, recordId, filterType, startDate, endDate) {
        var postUrl = GlobalVariables.baseAPIUrl + '/ajax_get_calendar_appointments';
        var postData = {
            'csrfToken': GlobalVariables.csrfToken,
            'record_id': recordId,
        ...
        $.post(postUrl, postData, function(response) {
            ...
            $.each(response.appointments, function(index, appointment) {
                var title = ((appointment['service']) ? appointment['service']['name'] : '') + '-' + ((appointment['customer']) ? appointment['customer']['fld_first_name'] : '') + '-' + ((appointment['customer']) ? appointment['customer']['fld_last_name'] : '')
                var event = {
                    'id': appointment['id'],
                    'title': title,
    ...
    saveAppointment: function(appointment, customer, successCallback, errorCallback) {
        var postUrl = GlobalVariables.baseAPIUrl + 'ajax_save_appointment';
    ...
    saveUnavailable: function(unavailable, successCallback, errorCallback) {
        var postUrl = GlobalVariables.baseAPIUrl + 'ajax_save_unavailable';
    ...
    calendarEventClick: function(event, jsEvent, view) {
        ...
        if ($parent.hasClass('fc-unavailable') || $altParent.hasClass('fc-unavailable')) {
            ...
            + event.data['provider']['fld_first_name'] + ' '
            + event.data['provider']['fld_last_name']
            + '<br>' +*/
            html =
                    '<style type="text/css">'
                        + '.popover-content strong {min-width: 80px; display:inline-block;}'
                    ...
                    '<strong>' + EALang['customer'] + '</strong> '
                        + event.data['customer']['fld_first_name'] + ' '
                        + event.data['customer']['fld_last_name']
                        + '<hr>' +
                    '<center>' +
                        '<button class="edit-popover btn btn-primary ' + displayEdit + '">' + EALang['edit'] + '</button>' +
                        '<button class="delete-popover btn btn-danger ' + displayDelete + '">' + EALang['delete'] + '</button>' +
    ...
    disableProviderSync: function(providerId) {
        ...
        // from the database.
        var postUrl = GlobalVariables.baseAPIUrl + 'ajax_disable_provider_sync';
        var postData = {
            'csrfToken': GlobalVariables.csrfToken,
        ...
        $.each(GlobalVariables.availableProviders, function(index, provider) {
            ...
            if (canProvideService) { // Add the provider to the listbox.
                var option = new Option(provider['first_name']
                       + ' ' + provider['last_name'], provider['fld_id']);
