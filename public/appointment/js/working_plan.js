...
WorkingPlan.prototype.bindEventHandlers = function() {
    ...
    $(document).on('click', '.save-break', function() {
        ...
        if (start > end) {
            $(this).parent().parent().find('.break-end  input').val(start.addHours(1).toString('h:mm tt'));
        }
...
WorkingPlan.prototype.get = function() {
    ...
    $('.working-plan input[type="checkbox"]').each(function() {
        ...
        if ($(this).prop('checked') == true) {
            workingPlan[id] = {};
            workingPlan[id].start = Date.parse($('#' + id + '-start').val()).toString('HH:mm');
            workingPlan[id].end = Date.parse($('#' + id + '-end').val()).toString('HH:mm');
